 ...
 *
 * 题目:  Problem 8-3
 * 名称:  Sorting variable-length items
 * 作者:  clrs.skanev.com
 * 语言:  c
 * 内容摘要: 变长数组的排序
 *
...
/* In order to simplify everything, both numbers and strings are
 * meshed in a single union called key_t. The key does not know
 * whether it is a number or a string - the handling code already
 * knows it instead. */
...
/* Performs counting sort on a dimension (number of digits or 
 * string length) and populates a table (groups) with the position
 * of each dimension. */
