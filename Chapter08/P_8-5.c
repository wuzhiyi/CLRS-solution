...
/* Average sorting is performed by just merge-sorting each column.
 * That was easy. Modifying merge sort was hard. */
...
/* Sorting a k-sorted array. We need to keep track of which column
 * produced the minimum element in the heap and this resulted in
 * quite the tricky C code. I don't think this is a good practice,
 * but still, that's the best I'm willing to make it right now. */
...
/* This is the merge sort from Chapter 2, modified to look only at
 * indices congruent to k modulo s. There are two very ugly and 
 * long macros that perform this unpleasant job. There's probably
 * a nicer way to do the calculation, but modular arithmetic has
 * always been my Achilles' heel. */

#define FIRST(index,k,s) ((index)+(s)-(index)%(k)+((index)%(k)<=(s)?0:(k)))
#define COUNT(a,b,k,s) ((b)-(a))/(k)+((((s)-(a)%(k))+(k))%(k)<((b)-(a))%(k)?1:0)) // 多了一个)?
...
/* Finally, the min heap from exercise 6.5-3, modified to store items
 * instead of ints. When I first wrote it, I made an error int the 
 * implementation and that sent me in an hour-long debugging session.
 * C is fun.
 * 
 * Also, there is a new heap operation (min_heap_push_pop) that is a
 * faster than heap_extract_min and then min_heap_insert. */
...
item heap_extract_min(heap_t *heap)
{
    ...
    heap->elments[0] = heap->elements[heap->heap_size-1];
    heap->heap_size--;
    min_heapify(heap, 0);
